...
## Set up
Download the environment.yml and supervised_classification.ipynb files and save them to your local machine. Open Anaconda Prompt and type in `conda env create -f <path/to/environment.yml>`. After all the packages install you can then run `conda activate my_599_env`. We now need to install ipykernel to make this easier by first running `conda install -y ipykernel` to ensure that it is installed. Then run `python -m  ipykernel install --user --name my_599_env --display-name "Python (599)`. Then to open jupyer lab run `jupyter lab`. Now you should be able to run all the code in the supervise_classification.ipynb.
